    ...
    const handleCheckboxChange = (index: number) => {
        setActiveCheckboxes(prev => {
            if (prev.includes(index)) {
                return prev.filter(i => i !== index); // اگر قبلاً انتخاب شده، آن را حذف می‌کند
            } else {
                return [...prev, index]; // اگر انتخاب نشده، به آرایه اضافه می‌کند
            }
        });
    };
                    ...
                    <tr>
                        {weekDays.map((item, index) => (
                            <td key={index} className="border px-4 py-2 text-center">
                                ...
                                {/* چک‌باکس برای هر روز */}
                                {activeCheckboxes.includes(index) && (
