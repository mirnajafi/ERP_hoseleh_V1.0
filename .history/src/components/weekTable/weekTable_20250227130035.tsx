...
const WeekTable: React.FC<WeekTableProps> = ({ apiUrl, selectedDay, onDaySelect }) => {
    ...
    const [showContent, setShowContent] = useState<boolean[]>(new Array(weekDays.length).fill(false)); // حالت نمایش محتوا برای هر روز
    ...
    // تابع برای نمایش/مخفی کردن محتوا در هر روز
                    ...
                    <tr>
                        {weekDays.map((item, index) => (
                            <td key={index} className="border px-4 py-2 text-center">
                                ...
                                {/* نمایش یا مخفی کردن محتوا بر اساس وضعیت */}
                                {showContent[index] && (
