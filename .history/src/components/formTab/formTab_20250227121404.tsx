...
const Tabs: React.FC<TabsProps> = ({ tabs, defaultIndex = 0 }) => {
  ...
  return (
    <div className="w-full overflow-hidden">
      <div className="flex py-3 overflow-x-auto whitespace-nowrap">
        {tabs.map((tab, index) => (
          <button
            key={index}
            className={`ml-2 p-1 text-nowrap text-xs font-bold text-sm focus:outline-none ${activeIndex === index
